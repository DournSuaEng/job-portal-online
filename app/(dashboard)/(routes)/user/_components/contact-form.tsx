...
import { Contact, Pencil, UserCircle } from 'lucide-react';
...
const formSchema = z.object({
  contact: z.string().min(1,{message : "contact number required"}),
});

const ContactForm = ({ initialData, userId }: ContactFormProps) => {
  const [isEditing, setIsEditing] = useState(false);
  const [name, setName] = useState(initialData?.contact || "");
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      setName(values.contact); // Update local state
  ...
  return (
    <Box>
      {!isEditing && (
        ...
        >
          <Contact className='w-6 h-6 mr-2 text-purple-500' />
          {name || 'No contact'}
        </div>
      )}
