...
const JobApllicantsPage =async ({params}:{params: {jobId: string}} ) => {
  ...
  const filteredProfiles = profiles &&  profiles.filter((profile)=> profile.appliedJobs.some((appliedJob) => appliedJob.jobId ===params.jobId));
  const formattedProfiles : ApplicantColumns[] = filteredProfiles.map(profile => ({
    ...
    appliedAt: profile.appliedJobs.find(job => job.jobId === params.jobId)?.appliedAt ? format(new Date(profile.appliedJobs.find(job => job.jobId === params.jobId)?.appliedAt ?? "") ,"MMM do, yyyy"): "",
...
export default JobApllicantsPage
