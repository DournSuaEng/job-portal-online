...
interface CompanyEditPageProps {
  params: Promise<{ companyId: string }>; // companyId is a string, not an array
}

export default async function CompanyEditPage({ params }: CompanyEditPageProps) {
  try {
    // Await the params to get the companyId
    ...
    const completionText = `(${completedFields}/${totalFields} - ${completionPercentage}%)`;

    return (
      <div className="p-6">
        <Link href="/admin/companies" className="flex items-center gap-3 text-sm text-neutral-500 hover:text-neutral-700 transition">
    ...
    console.error("[CompanyEditPage] Error:", error);
