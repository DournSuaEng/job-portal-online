...
export const PATCH = async (
  req: Request,
  { params }: { params: Promise<{ companyId: string }> } // Correct type with Promise
) => {
  try {
    // Authenticate the user and get their ID
    const { userId } = await auth(); // Already async, no change needed

    // Await params to resolve the Promise and destructure companyId
